...
pragma optional_param max_backends 2;
pragma optional_param customer_id "3LabtymqgH1aISLQX7fKdL";
C!
W!
# Backends

backend F_mareedeslettres_herokuapp_com__Heroku_ {
    .connect_timeout = 1s;
    .dynamic = true;
    .port = "80";
    .host = "mareedeslettres.herokuapp.com";
    .first_byte_timeout = 15s;
    .max_connections = 200;
    .between_bytes_timeout = 10s;
    .share_key = "1rCX4awk5ydET4mYw5WurW";
  
      
    .probe = {
        .request = "HEAD / HTTP/1.1"  "Host: mareedeslettres.herokuapp.com" "Connection: close";
        .window = 5;
        .threshold = 1;
        .timeout = 2s;
        .initial = 5;
...
#--FASTLY RECV BEGIN
  if (req.restarts == 0) {
    ...
    set req.http.host = "mareedeslettres.herokuapp.com";
        
  # default conditions
  set req.backend = F_mareedeslettres_herokuapp_com__Heroku_;
...
# generic mechanism to vary on something
 ...
 # priority: 0
  ...
  # Header rewrite Remove Set Cookie : 10
...
#--FASTLY LOG START
  ...
  # logentries Logentries_one_click
  log {"syslog 1rCX4awk5ydET4mYw5WurW Logentries one-click :: "} {""client_ip" = ""} req.http.Fastly-Client-IP {"", "timestamp" = ""} now {"", "request" = ""} req.request {"", "url" = ""} req.url {"", "grace" = ""} req.grace {"", "status" = ""} resp.status {"", "response" = ""} resp.response {"", "size" = ""} req.bytes_read {"", "header_size" = ""} req.header_bytes_read {"", "body_size" = ""} req.body_bytes_read {"", "city" = ""} geoip.city {"", "continent" = ""} geoip.continent_code {"", "postalcode" = ""} geoip.postal_code {"", "POP" = ""} server.datacenter {"", "server_region" = ""} server.region {"", "region" = ""} geoip.region {"", "country" = ""} geoip.country_code {"", "countryName" = ""} geoip.country_name {"", "referrer" = ""} req.http.referer {"", "devicetype" = ""} req.http.User-Agent {"", "numberOfHits" = ""} obj.hits {"", "restarts" = ""} req.restarts {"", "responsetime" = ""} time.elapsed.sec {"""};
